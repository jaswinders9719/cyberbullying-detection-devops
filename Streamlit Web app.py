...
@st.cache_resource
def load_classical(
    best_model_path="artifacts_classical/best_model.pkl",
    encoder_path="artifacts_classical/label_encoder.pkl"
):
...
tox_vec = None
tox_reg = None
tox_ok = os.path.exists("vectorizer.pkl") and os.path.exists("reg_multi_model.pkl")

if tox_ok:
    tox_vec, tox_reg = load_toxicity_artifacts(
        "vectorizer.pkl",
        "reg_multi_model.pkl"
    )
else:
    st.sidebar.warning(
        "Toxicity files not found: vectorizer.pkl and/or reg_multi_model.pkl"
    )
